...
tmux set-option -ga status-right "#[fg=${cp_bg}]${right_sep}#[fg=${cp_fg},bg=${cp_bg}] #(test "#{pane_current_path}" = "$HOME" && echo '~' || basename "#{pane_current_path}") #[fg=${ram_bg}]${right_sep}#[fg=${ram_fg},bg=${ram_bg}] RAM: #{ram} #[fg=${cpu_bg}]${right_sep}#[fg=${cpu_fg},bg=${cpu_bg}] CPU: #{cpu_percentage} #[fg=${time_bg}]${right_sep}#[fg=${time_fg},bg=${time_bg}] %a %b %d %I:%M"
